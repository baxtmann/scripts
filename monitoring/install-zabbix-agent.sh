...
if [ -f /etc/zabbix/zabbix_agent2.conf ]; then #CHANGE THIS ONCE INITIAL TESTING IS DONE TO AGENT2 PATH
    ...
    if [ "$(uname -m)" = "armv7l" ] && [ "$(grep -E 'Raspbian' /etc/os-release)" ]; then
        os="raspbian"
    elif [ "$ubuntu_version" = "18.04" ] || [ "$ubuntu_version" = "22.04" ] || [ "$ubuntu_version" = "20.04" ]; then
        os="ubuntu"
    else
        echo "Error: This script is only compatible with Ubuntu 18.04, 20.04, 22.04 and Raspbian on ARM architecture."
        exit 1
    fi
